...
def create_db(db_path: Path) -> sqlite3.Connection:
    ...
    if db_path.exists():
        db_path.unlink()
        logger.info("Removed existing DB: %s", db_path.name)
    conn = sqlite3.connect(db_path)
    conn.executescript(DDL)
    conn.commit()
    logger.info("DB created: %s", db_path)
    return conn
...
def load_dim_date(conn: sqlite3.Connection) -> None:
    ...
    conn.commit()
    logger.info("dim_date: %d rows (2022-01-01 → 2026-12-31)", len(rows))
...
def load_dim_grower(conn: sqlite3.Connection, csv_dir: Path) -> None:
    ...
    conn.commit()
    logger.info("dim_grower: %d rows", len(rows))
...
def load_dim_corridor(conn: sqlite3.Connection) -> None:
    ...
    conn.commit()
    logger.info("dim_corridor: %d rows", len(CORRIDORS))
...
def load_dim_variety(conn: sqlite3.Connection) -> None:
    ...
    conn.commit()
    logger.info("dim_variety: %d rows", len(VARIETIES))
...
def load_dim_packhouse(conn: sqlite3.Connection) -> None:
    ...
    conn.commit()
    logger.info("dim_packhouse: %d rows", len(PACKHOUSES))
...
def _build_variety_lut(conn: sqlite3.Connection) -> dict[str, int]:
    ...
    return dict(
        conn.execute(
            "SELECT LOWER(variety_name), variety_id FROM dim_variety"
        ).fetchall()
    )
...
def load_fact_pallet_submissions(conn: sqlite3.Connection, csv_dir: Path) -> None:
        ...
        for raw in csv.DictReader(fh):
            try:
                ...
                if grower_id not in grower_ids:
                    logger.warning("kpin %d not in dim_grower — row skipped", grower_id)
                    skipped += 1
                    continue

                date_id = date_lut.get(sub_date)
                if date_id is None:
                    logger.warning("date '%s' not in dim_date — row skipped", sub_date)
                    skipped += 1
                    continue

                variety_name = VARIETY_MAP.get(var_key)
                if variety_name is None:
                    logger.warning("unknown variety '%s' — row skipped", raw["variety"])
                    skipped += 1
                    continue
                ...
                logger.warning("Malformed row — %s: %s", exc, raw)
                skipped += 1
    ...
    conn.commit()
    logger.info(
        "fact_pallet_submissions: %d rows loaded, %d skipped", len(rows), skipped
    )
...
def validate(conn: sqlite3.Connection) -> None:
    """Log row counts, check FK integrity, and verify no NULLs in NOT NULL columns."""
    tables = [
        "dim_date",
        "dim_grower",
        "dim_corridor",
    ...
    for t in tables:
        ...
        logger.info(f"  {t:<36} {n:>8,}")

    logger.info("── FK integrity ──")
    violations = conn.execute("PRAGMA foreign_key_check").fetchall()
    if violations:
        logger.warning(f"  {len(violations)} FK violation(s):")
        for v in violations:
            logger.warning(f"     {v}")
    else:
        logger.info("  0 FK violations")

    logger.info("── NOT NULL columns (fact) ──")
    not_null_cols = [
        "grower_id",
        "corridor_id",
        "date_id",
    ...
    for col in not_null_cols:
        ...
        if n:
            logger.warning(f"  {col}: {n} NULL(s)")
            any_nulls = True
    if not any_nulls:
        logger.info("  0 NULLs in NOT NULL columns")
...
def main() -> None:
    ...
    logger.info("=== APOPHENIA Star Schema Loader — Start ===")
    t0 = datetime.now()
