...
from config import SYNTHETIC_EDI_DIR, configure_logging  # noqa: E402
...
def generate_grower_register() -> pd.DataFrame:
    ...
    logger.info(
        f"synthetic_grower_register.csv — {len(df)} growers | "
...
def generate_maturity_readings(growers: pd.DataFrame) -> pd.DataFrame:
    ...
    grower_effects = {
        row["kpin"]: float(rng.normal(0, 0.25)) for _, row in growers.iterrows()
    }
    ...
    for season, s in SEASONS.items():
        for _, g in growers.iterrows():
            ...
            for pw in PACK_WEEKS:
                ...
                dm = round(
                    float(
                        np.clip(dm_raw + grower_effects[kpin] - psa_penalty, 12.0, 22.0)
                    ),
                    2,
                )
    ...
    logger.info(
        f"synthetic_maturity_readings.csv — {len(df):,} readings | "
...
def generate_pallet_submissions(
    growers: pd.DataFrame, maturity: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    for season, s in SEASONS.items():
        ...
        for _, g in growers.iterrows():
            ...
            for pw in hw:
                ...
                trays_sub = max(
                    50, int(ha * y_week * weekly_frac * (s["vol_index"] / 100))
                )
                ...
                # Payments
                rate = (
                    SUBMIT_RATES["Green_sz42"]
                    if (variety == "Green" and size == "42")
    ...
    logger.info(
        f"synthetic_pallet_submissions.csv — {len(df):,} submissions | "
...
def generate_fruit_loss_records(submissions: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(
        f"synthetic_fruit_loss_records.csv — {len(df):,} loss events | "
...
def main() -> None:
    ...
    """
    logger.info("=" * 70)
    logger.info("APOPHENIA — EDI Simulation Data Generation  [v2 — recalibrated]")
    logger.info("Industry Quality Manual 2026 | Grower Payments Booklet 2026")
    logger.info("Seasons: 2022/23 → 2025/26 | Pack weeks 11–26")
    logger.info("Author: Gabriela Olivera | Data Analytics Portfolio")
    logger.info("=" * 70)

    logger.info("── TABLE 1: Grower Register ──")
    growers = generate_grower_register()

    logger.info("── TABLE 2: Maturity Readings ──")
    maturity = generate_maturity_readings(growers)

    logger.info("── TABLE 3: Pallet Submissions ──")
    submissions = generate_pallet_submissions(growers, maturity)

    logger.info("── TABLE 4: Fruit Loss Records ──")
    ...
    logger.info("MTS Compliance by Season (target: 88-94%):")
    mts_season = maturity.groupby("season")["mts_pass"].mean() * 100
    for s, pct in mts_season.items():
        flag = "OK" if 85 <= pct <= 97 else "CHECK"
        logger.info(f"  [{flag}] {s}: {pct:.1f}% pass")

    logger.info("DM% by Subzone — mean ± std (all seasons):")
    dm_sz = maturity.groupby("subzone")["dm_pct"].agg(["mean", "std"])
    dm_sz = dm_sz.sort_values("std")
    for sz, row in dm_sz.iterrows():
        bar = "#" * int(row["std"] * 20)
        logger.info(
            f"  {sz:<12}  mean={row['mean']:.2f}%  std={row['std']:.2f}%  {bar}"
        )
    logger.info("  (lower std = more consistent = lower risk)")

    logger.info("Total Returns by Season (NZD M):")
    ...
    for s in ret_season.index:
        logger.info(
            f"  {s}: NZD {ret_season[s]:,.1f}M  |  "
            f"{trays_season[s]:,.0f} trays submitted"
        )

    logger.info("Loss Cause Distribution:")
    ...
    for cause, count in cause_dist.items():
        ...
        logger.info(f"  {cause:<22} {count:>5,}  ({pct:.1f}%)")
    ...
    logger.info("All KPINs are fictional. No real proprietary grower data used.")
    logger.info("Distributions calibrated from industry 2026 public standards only.")
    logger.info("=" * 70)
